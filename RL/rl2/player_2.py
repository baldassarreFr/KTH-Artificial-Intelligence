...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # if all_actions is empty, return None (shouldn't happen in normal flow)
        if not all_actions:
            return None

        # Exploration with probability epsilon: pick uniformly among allowed actions
        if np.random.rand() < epsilon:
            action = np.random.choice(all_actions)
            return int(action)

        # Exploitation: pick best action among allowed actions according to Q
        try:
            # Extract Q-values for the allowed actions
            q_vals = Q[state, all_actions]
            # If all q_vals are nan (rare), fallback to random choice
            if np.all(np.isnan(q_vals)):
                action = np.random.choice(all_actions)
            else:
                # np.nanargmax returns index within q_vals array
                best_idx = int(np.nanargmax(q_vals))
                action = int(all_actions[best_idx])
        except Exception:
            # In case Q or indexing fails for any reason, fallback to random allowed action
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        # Initialize a numpy array with ns state rows and na columns with float values from 0.0 to 1.0.
        ...
        while episode <= self.episode_max and diff > self.threshold:
            ...
            while not end_episode:
                ...
                # Chose an action from all possible actions with the highest Q value
                actions_pool = [Q[s_current, i] if i in list_pos else np.nan for i in range(na)]
                action = np.nanargmax(actions_pool)
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
    ...
    def random_agent(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                actions_pool = self.allowed_moves[s_current]
