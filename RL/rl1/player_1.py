...
class PlayerController(SettingLoader, Communicator):
    ...
    # Impede movements that would take the diver out of the grid
    def allowed_movements(self):
        ...
        for s in self.ind2state.keys():
            ...
            if s[0] < self.space_subdivisions - 1:
                self.allowed_moves[self.ind2state[s]] += [1]    # right
            if s[0] > 0:
                self.allowed_moves[self.ind2state[s]] += [0]    # left
            if s[1] < self.space_subdivisions - 1:
                self.allowed_moves[self.ind2state[s]] += [3]    # up
            if s[1] > 0:
                self.allowed_moves[self.ind2state[s]] += [2]    # down
...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ns = len(self.state2ind.keys()) # number of states
        na = len(self.actions.keys())   # number of actions

        init_pos_tuple = self.settings.init_pos_diver
        init_pos = self.ind2state[(init_pos_tuple[0], init_pos_tuple[1])]   # initial position of the diver in index format
        episode = 0
        R_total = 0
        steps = 0
        current_total_steps = 0
