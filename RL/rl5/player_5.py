...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        # EXPLOITATION (chose the best known action among allowed ones)
...
def anneal_alpha(initial_alpha, final_alpha, current_total_steps, anneal_timesteps):
    if current_total_steps >= anneal_timesteps:
        alpha_t = final_alpha
    else:
        delta_alpha = final_alpha - initial_alpha
        alpha_t = initial_alpha + delta_alpha * (current_total_steps / anneal_timesteps)
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        rewards_window = deque(maxlen=self.moving_average_window)
        rewards_window_converged = False
        avg_convergence_threshold = 1
        window_avg = 0
        highest_R = float('-inf')
        
        best_Q = Q.copy()
        best_policy = self.get_policy(Q)
        ...
        while episode <= self.episode_max and diff > self.threshold and not rewards_window_converged:
            ...
            while not end_episode:
                ...
                lr = anneal_alpha(self.alpha_initial, self.alpha_final, current_total_steps, self.annealing_timesteps)
            ...
            # end episode -> update diff
            diff = np.abs(np.nanmean(Q - Q_old))
            rewards_window.append(R_total)
            
            if len(rewards_window) == self.moving_average_window:
                window_avg = np.mean(rewards_window)
                highest_R = np.max(rewards_window)
                
                if highest_R == R_total:
                    best_Q = Q.copy()
                    best_policy = self.get_policy(best_Q)

                if (highest_R - window_avg) <= avg_convergence_threshold:
                    rewards_window_converged = True                
            ...
            print(
                "Episode: {}, Steps {}, Diff: {:6e}, Total Reward: {}, Total Steps {}, Window Avg: {}"
                .format(episode, steps, diff, R_total, current_total_steps, window_avg))
            
            episode += 1
            end_episode = False

        # return best_Q if available else final Q
...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        # Initialize a numpy matrix with ns state rows and na state columns with zeros
        n=np.zeros((ns, na)) # counter of actions per each state
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                n[s_current, action] += 1 # update counter of actions per state
